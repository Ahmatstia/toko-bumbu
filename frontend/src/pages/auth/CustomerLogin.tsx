...
const CustomerLogin: React.FC = () => {
  ...
  const onSubmit = async (data: CustomerLoginForm) => {
    setIsLoading(true);
    try {
      const response = await authService.customerLogin(data);
      
      toast.success(`Selamat datang, ${response.customer.name}!`);
      navigate('/');
    } catch (error: any) {
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 text-center p-4 border border-gray-200 rounded-lg bg-white">
              <p className="font-semibold text-gray-800">Customer Demo</p>
              <p className="text-sm text-gray-600 mt-1">budi@example.com</p>
