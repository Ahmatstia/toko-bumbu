...
const Home: React.FC = () => {
  ...
  // Fetch products - HANYA PRODUK AKTIF (isPublic=true)
  const {
    data: productsData,
    ...
    queryFn: async () => {
      const response = await api.get("/products/public?limit=8");
      return response.data.data || [];
    },
  });

  // Fetch stock for products
  const [productsWithPrice, setProductsWithPrice] = React.useState<
    ProductWithPrice[]
  >([]);
